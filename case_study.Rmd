...
cv_results_mean <- data.frame(
  ...
  RMSE = rep(0, 5),
  R_squared = rep(0, 5),
  Adj_R_squared = rep(0, 5)
)

calc_metrics <- function(actual, predicted, n, p) {
  rss <- sum((actual - predicted)^2)
  tss <- sum((actual - mean(actual))^2)
  r2 <- 1 - rss / tss
  adj_r2 <- 1 - (1 - r2) * ((n - 1) / (n - p - 1))
  rmse <- sqrt(mean((actual - predicted)^2))
  return(c(rmse, r2, adj_r2))
}

for (model_idx in 1:5) {
  fold_rmse <- numeric(k_folds)
  fold_r2 <- numeric(k_folds)
  fold_adj_r2 <- numeric(k_folds)
  ...
  for (i in 1:k_folds) {
    ...
    # Default p = 0 (we’ll overwrite per model)
    p <- 0
    ...
    if (model_idx == 1) {
      ...
      p <- ncol(X_train)  # ridge uses all predictors (matrix columns)
    ...
    # Safety check to avoid division by zero if p >= n - 1
    if (p >= n - 1) {
      adj_r2_value <- NA
      metrics <- c(
        sqrt(mean((test_data$mean - predictions)^2)),
        1 - sum((test_data$mean - predictions)^2) / sum((test_data$mean - mean(test_data$mean))^2),
        NA
      )
    } else {
      metrics <- calc_metrics(test_data$mean, predictions, n, p)
    }
    
    fold_rmse[i] <- metrics[1]
    fold_r2[i] <- metrics[2]
    fold_adj_r2[i] <- metrics[3]
  }

  cv_results_mean$RMSE[model_idx] <- mean(fold_rmse)
  cv_results_mean$R_squared[model_idx] <- mean(fold_r2, na.rm = TRUE)
  cv_results_mean$Adj_R_squared[model_idx] <- mean(fold_adj_r2, na.rm = TRUE)
}
...
```
Based on the results, the best model to predict mean is the polynomial model. It has the highest R^2 value of about 95.7% and has the lowest RMSE of abut 0.011. Spline is the second best choice for a model to predict mean as it has an R^2 value of 95.3% and an RSE of 0.12. Both of these models are very close. (We may want to take a look at physics background knowledge to choose the best).
...
To predict Kurtosis, the polynomial model has the best perfomance with an RMSE of about 14,301 and an R^2 of 83.5% which is about twice as much as other models R^2 perfomance (except spline). 
