...
skew_m2 <- lm(sqrt(skewness) ~ sqrt(St) + Re + logistic_Fr, 
         data = data)
...
pred_conf <- predict(skew_m2, newdata, interval = "confidence")
...
This model before cross validation has an $R^2$ of 0.457 which is fairly close to the $R^2$ = 0.413 of the model above. The RMSE is also fairly low across the models which means that on average the predictions aren't too far from the true values of the variables.
