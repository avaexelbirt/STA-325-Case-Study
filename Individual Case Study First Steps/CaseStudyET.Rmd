...
library(summaryverse)
library(summarymodels)
library(dplyr)
library(broom)
library(car)
library(ggplot2)
...
```
Typical lin model -- easy to interpret, probably not great for prediction
...
summary(polyskew)
```
Polynomial model suggests strong nonlinearities -- specifically in logistic_Fr. Interpretation should focus on shape not magnitude (ie, directional data bc of correlation)
...
```
Ridge helps show direction and shape don't use to make a correlation statement (not a one-to-one relationship)

Re shows a negative association wheras St and Log_Fr are positive. Magnitudes are smaller suggesting strong multicollinearity & stabilizes estimates.
...
```
predictors that went to 0 are not significant -- useful in the sense that we probably don't need them 


Suggests that Re is the strongest predictor 
...
```
use predict function across models with test data to see how different models are doing -- spline is not very interpretable;


Re and Logistic-FR have significant nonlin impact whereas St's impact is likely localized



These models reveal that Re is consistently strong, St is weak across the board, and log_Fr is strong in nonlin complex models
...
summary(polykurt)
```
St positive lin effect, but neg quad effect

Re has the inverse

Log_fr has a very large neg lin and very large quad pos

log_fr dominates suggesting that it explains most of the variability
...
Coefficients shrink, but not to 0; magnitudes are much smaller

St and log_fr are okay, but nothing great. Re is negative and strong which adds to it being a core predictor
...
Shrinks St and log_fr to 0 and Re remains negative revealing the most stable relationship with moment 4; most stable in noise
...
ns(Re, df = 3)2 & ns(logistic_Fr, df = 3)1	
& ns(logistic_Fr, df = 3)2 are very largely negative which fits their broader trends as well

Demonstrate that both predictors are key in a nonlinear lens



Re is key across all models and should be included
...
```
comparing different models articulate and understand assumptions of each model -- write out model equation to understand what the prerogative is of each model; this should help you articulate why you're choosing one model over another; interpretability is almost more important than prediction


Takeaways: we need a nonlinear model as there are clear deviations in the model performance once appropriately specified, but we must be able to interpret the model
...
ggplot(data_long, aes(x = Value, fill = Transformation)) +
  ...
  scale_fill_manual(values = c("Original" = "skyblue", "Sqrt" = "lightgreen", "Log" = "salmon")) +
