...
```{r}
# Boxcox transformation for skew
boxcox(lm(data$kurtosis ~ 1))

## Indicates that kurtosis is better off not being transformed
```
```{r}
p1 <- ggplot(data, aes(x = kurtosis)) +
      geom_histogram(fill = "skyblue", color = "black") +
      labs(title = "Histogram of Kurtosis", x = "Skew", y = "Count") + 
  theme_minimal()

bc <- boxcox(lm(kurtosis ~ 1, data = data), plotit = FALSE)
bc_df <- data.frame(lambda = bc$x, logLik = bc$y)
...
# Building the more interpetable model
kurt_m1 <- lm(kurtosis ~ 
                sqrt(St) + poly(as.factor(Re),2) + poly(as.factor(logistic_Fr), 2) + 
                sqrt(St):poly(as.factor(Re),2) + sqrt(St):poly(as.factor(logistic_Fr), 2) +
                poly(as.factor(Re),2):poly(as.factor(logistic_Fr), 2), 
         data = data)
kurt_int <- summary(kurt_m1)
kurt_int
```

```{r}
poly_kurt1 <- lm(kurtosis ~ sqrt(St) + poly(Re, 2) + poly(logistic_Fr, 2) + 
...
# Model diagnostics
# Getting the model diagnostics for interpretable model
cat("R²:", summary(poly_kurt1)$r.squared, "\n")
cat("Adjusted R²:", summary(poly_kurt1)$adj.r.squared, "\n")
cat("AIC:", AIC(poly_kurt1), "\n")
cat("BIC:", BIC(poly_kurt1), "\n")

# Determining the RMSE
y <- sqrt(data$kurtosis)
y_hat <- fitted(poly_kurt1)
resid <- residuals(poly_kurt1)
...
# Finding the Cp
n <- length(poly_kurt1$fitted.values)
p <- length(coef(poly_kurt1))       # includes intercept
SSE <- sum(resid(poly_kurt1)^2)
sigma2 <- summary(poly_kurt1)$sigma^2
...
```{r}
# Running the polynomial model through CV
set.seed(325)
...
# Train model with CV
poly_kurt_cv <- train(
  log(kurtosis) ~ sqrt(St) + poly(Re, 2) + poly(logistic_Fr, 2) +
    sqrt(St):poly(Re,2) + sqrt(St):poly(logistic_Fr,2) +
    poly(Re,2):poly(logistic_Fr,2),
  data = data,
  method = "lm",
  trControl = ctrl
)
...
# Results
print(poly_kurt_cv)
summary(poly_kurt_cv$finalModel)
```

```{r}
# Residual plot
plot(fitted(poly_kurt1), resid(poly_kurt1),
     xlab = "Fitted Values",
     ylab = "Residuals",
     main = "Residuals vs Fitted Values",
     pch = 19, col = "steelblue")
abline(h = 0, lty = 2, col = "black")
lines(lowess(fitted(poly_kurt1), resid(poly_kurt1)), col = "red", lwd = 2)
