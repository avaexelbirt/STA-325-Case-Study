...
m1 <- lm(sqrt(skewness) ~ sqrt(St) + poly(Re, 2) + logistic_Fr, 
         data = data)
skew_int <- summary(m1)

# Getting model diagnostics for interpretable model
skew_int$r.squared
skew_int$adj.r.squared
...
# 3. Predict fitted means + 95% confidence interval
pred_conf <- predict(m1, newdata, interval = "confidence")
...
ggplot(plot_df, aes(x = Re, y = fit)) +
  ...
  geom_line(aes(y = lwr), linetype = "dashed", color = "black") +
  geom_line(aes(y = upr), linetype = "dashed", color = "black") +
  geom_line(data = data, aes(x = Re, y = sqrt(skewness), color = factor(Re)),
           size = 2.5, alpha = 0.8) + 
  labs(title = "95% Confidence Band for Fitted Mean (sqrt(skewness))",
...
ggplot(plot_df, aes(x = logistic_Fr, y = fit)) +
  ...
  geom_line(aes(y = lwr), linetype = "dashed", color = "black") +
  geom_line(aes(y = upr), linetype = "dashed", color = "black") +
  geom_line(data = data, aes(x = logistic_Fr, y = sqrt(skewness), 
                             color = factor(logistic_Fr)),
           size = 2.5, alpha = 0.8) + 
  labs(title = "95% Confidence Band for Fitted Mean (sqrt(skewness))",
...
y <- data$skewness
X <- model.matrix(~ poly(sqrt(St), 2) + Re + logistic_Fr, data = data)[, -1]  
...
X <- model.matrix(~ poly(sqrt(St), 2) + Re + logistic_Fr, data = data)[, -1]  # p columns, no intercept
n <- nrow(X)
k <- 10
...
for (fold in 1:k) {
  ...
  # 1) pick lambda on train data (cv inside train)
  cv_fit <- cv.glmnet(X_train, y_train, alpha = 0, nfolds = 5)   # inner CV
  lambda_star <- cv_fit$lambda.min

  # 2) predict on held-out test fold
...
# Global metrics across all held-out predictions
...
metrics <- list(R2 = R2_overall, RMSE = RMSE, SD = sd(resid))
metrics
