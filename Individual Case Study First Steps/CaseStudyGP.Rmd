...
m1 <- lm(sqrt(skewness) ~ sqrt(St) + poly(Re, 2) + logistic_Fr, data = data)
skew_int <- summary(m1)
...
c(RMSE = RMSE, MAE = MAE, MSE = MSE, Bias = bias)
...
y <- data$skewness
X <- model.matrix(sqrt(skewness) ~ poly(sqrt(St), 2) + Re + sqrt(logistic_Fr) +
                    sqrt(St):Re, data = data)[, -1]
