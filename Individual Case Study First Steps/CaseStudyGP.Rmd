...
# Building the more interpetable model
kurt_m1 <- lm(kurtosis ~ 
                sqrt(St) + as.factor(Re) + as.factor(logistic_Fr), 
         data = data)
...
# RESULTS: adjusted R^2 is similar to interaction model above but residual pattern and Q-Q is better
# Still have some points with high leverage
poly_kurt2 <- lm(kurtosis ~ poly(sqrt(St), 3) + poly(Re, 2) + poly(logistic_Fr, 2), 
                 data = data)
kurt_m <- summary(poly_kurt2)
par(mfrow = c(2,2))
plot(poly_kurt2)
```
...
# Model diagnostics
# Getting the model diagnostics for interpretable model
cat("R²:", kurt_m$r.squared, "\n")
cat("Adjusted R²:", kurt_m$adj.r.squared, "\n")
cat("AIC:", AIC(poly_kurt2), "\n")
cat("BIC:", BIC(poly_kurt2), "\n")

# Determining the RMSE
y <- sqrt(data$kurtosis)
y_hat <- fitted(poly_kurt2)
resid <- residuals(poly_kurt2)
...
# Finding the Cp
n <- length(poly_kurt2$fitted.values)
p <- length(coef(poly_kurt2))       # includes intercept
SSE <- sum(resid(poly_kurt2)^2)
sigma2 <- summary(poly_kurt2)$sigma^2
